...
class TPersonal_model extends Conexion {
	private $tabla="tpersonal";
	private $id;
	private $id_entidad;
	private $descripcion;
	private $id_user;
	...
	function consultaModel(){
		return $this->db->select1("id_tipo_personal, descripcion",$this->tabla,"id_tipo_personal=$this->id");
	}
	...
	function update(){
		$query="update $this->tabla set descripcion  = '$this->descripcion',
                                        updated_at  = now()
		        where id_tipo_personal=$this->id ";
		return $this->db->ejecutaMdl($query);
	}
