<?php 
class MarcaVehiculo_model extends Conexion {
    ...
    function consultaModel(){
	 return $this->db->select1(" id_marca_vehiculo, descripcion",$this->tabla,"id_marca_vehiculo=$this->id");
    }
    
    function listadoModel($id_entidad){ 
        $sql="select id_marca_vehiculo, descripcion 
              from $this->tabla 
    ...
    function update(){
		$query="update $this->tabla set descripcion = '$this->descripcion',
                                        updated_at  = now()
		        where id_marca_vehiculo=$this->id ";
		return $this->db->ejecutaMdl($query);
	}
